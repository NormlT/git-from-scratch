...
#
# DELIBERATELY BAD: this script commits a placeholder SECRET_KEY string into
# the practice repo so `git bisect` can later identify which commit
# introduced the issue. The string is FAKE -- it is not a real credential
# and is only written into the disposable /tmp/git-lab-10/ practice repo.
# Credential scanners will flag it; that flag is a false positive in this
# educational context (see .gitleaks.toml allow-list and cyber-neo CN-002).
set -euo pipefail
